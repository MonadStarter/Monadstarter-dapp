...
contract SeedifyLaunchpad is Ownable, Pausable {
    ...
    error ValueTooLow(uint256 val, uint256 min);
    error ValueTooHigh(uint256 val, uint256 max);
    error SaleNotEnded();
    error NotRefundable(uint256 softCap, uint256 amountRaised);
    error NotEnoughBalance(address sender, uint256 amount);
  //  error SaleNotStarted();

    string public name;
    uint256 public softCap; //need to implement this
    ...
    constructor(
        ...
        name = _name;
        if (_maxCap <= 0) {
            revert ValueTooLow(_maxCap, 1000);
        }

        maxCap = _maxCap;
        if (_softCap <= 0) {
            revert ValueTooLow(_softCap, 1000);
        }
        if (_softCap >= _maxCap) {
            revert ValueTooHigh(_softCap, _maxCap);
        }
        softCap = _softCap;
    ...
    //refund all money if softcap has not been reached after the sale end time has passed
    // can be called by anyone after the sale has ended
    //TODO: ensure rentry attacks
    // TODO: discuss if we need to implement this
    function refundAll() public SaleEnded {
        //check if soft cap has been reached or not
        if (totalBUSDReceivedInAllTier >= softCap){
            revert NotRefundable(softCap, totalBUSDReceivedInAllTier)
        }

        // if there's any money left to refund
        if (totalBUSDReceivedInAllTier <= 0){
            revert NotEnoughBalance(address(this), totalBUSDReceivedInAllTier);
        }

        //process refund for all
    ...
    // refunds the amount if sale has ended and soft cap has not reached
    function refund() public SaleEnded {

        if (totalBUSDReceivedInAllTier >= softCap){
            revert NotRefundable(softCap, amountRaised);
        }

        User storage user = userDetails[msg.sender];
        uint256 amountToRefund = user.investedAmount;
        
        if (amountToRefund <= 0){
            revert NotEnoughBalance(msg.sender, amountToRefund);
        }
        
        // Transfer the invested amount back to the user
        //TODO: currently tranfering from projectOwner won't work if they revoke permision or we haven't asked for permission
        // Update this so the token is in contract
        _transferTokens(projectOwner, msg.sender, amountToRefund);

        // Update the total BUSD received in all tiers
        totalBUSDReceivedInAllTier -= amountToRefund;

        // Reset the user's invested amount
        user.investedAmount = 0;

        // Emit a refund event
        emit UserRefunded(msg.sender, amountToRefund);
    }
    ...
    function buyTokens(
        ...
        //TODO: currently directly goes to project owner
